...
onValue(
  ...
  (snapshot) => {
    ...
    // Treat "no data yet" as available so the app works before a driver
    // ever logs in and sets a status.
...
function renderDriverStatus() {
  ...
  driverCurrentStatus.textContent = coasterAvailable
    ? "Current status: ✅ Available"
    : "Current status: 🚫 Not available";
  driverCurrentStatus.style.color = coasterAvailable ? "#18785F" : "#A8371F";
}
...
function showDriverPanel() {
  ...
  driverLoginBtn.hidden = true;
  renderDriverStatus();
}

function hideDriverPanel() {
  driverPanel.hidden = true;
  driverLoginBtn.hidden = false;
}

driverLoginBtn.addEventListener("click", openLoginModal);
...
// Restore driver panel visibility on page load/refresh.
if (isDriverLoggedIn) {
  showDriverPanel();
} else {
  hideDriverPanel();
...
function render() {
  ...
  availabilityBanner.hidden = coasterAvailable;
  if (!coasterAvailable) {
