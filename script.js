...
const availabilityRef = ref(db, "coasterAvailability");

// ================= DRIVER LOGIN =================
// NOTE: This is a simple client-side gate for a class project. The
// credentials live in this file, so anyone who views the page source can
// read them — it is not real authentication/security.
const DRIVER_EMAIL = "coastermcc@gmail.com";
const DRIVER_PASSWORD = "123456789";
const DRIVER_SESSION_KEY = "coaster_driver_session_v1";
const ROLE_SESSION_KEY = "coaster_role_v1"; // "student" | "driver"
...
let coasterAvailable = true; // assumed available until Firebase says otherwise
let coasterFull = false; // manual driver override, independent of seat count
let isDriverLoggedIn = sessionStorage.getItem(DRIVER_SESSION_KEY) === "true";
...
const firebaseErrorBox = document.getElementById("firebase-error");
const firebaseErrorMessageEl = document.getElementById("firebase-error-message");
...
// Sections that only make sense for a student, hidden while a driver is
// logged in so the driver gets a focused dashboard.
const tripSectionEl = document.getElementById("trip-section");
...
onValue(
  ...
  (error) => {
    ...
    firebaseReady = false;
    firebaseError = true;
    firebaseErrorMessage = error?.code === "PERMISSION_DENIED"
      ? "Firebase rejected access. Update your Realtime Database rules to allow this reservation page to read and write coasterReservations."
      : (error?.message || "Unable to connect to the reservation database.");
    render();
  }
);
...
onValue(
  ...
  (snapshot) => {
    ...
    // Treat "no data yet" as available/not-full so the app works before a
    // driver ever logs in and sets a status.
    coasterAvailable = value ? value.available !== false : true;
    coasterFull = value ? value.full === true : false;
    renderDriverStatus();
    render();
  },
  (error) => {
...
// update() merges just the field being changed instead of overwriting the
// whole coasterAvailability object, so toggling "full" never clobbers
// "available" and vice versa.
async function setCoasterAvailability(isAvailable) {
  if (!firebaseReady) {
    showToast("Live sync isn't ready yet. Try again in a moment.");
    return;
  }
  ...
  try {
    await update(availabilityRef, {
      ...
      updatedBy: DRIVER_EMAIL,
    });
    showToast(
      isAvailable
        ? "Marked the coaster as available."
        : "Marked the coaster as NOT available."
    );
  } catch (err) {
    console.error("Failed to update availability:", err);
    showToast("Could not update coaster availability.");
  }
}

async function setCoasterFull(isFull) {
  if (!firebaseReady) {
    showToast("Live sync isn't ready yet. Try again in a moment.");
    return;
  }
  ...
  try {
    await update(availabilityRef, {
      full: isFull,
      fullUpdatedAt: serverTimestamp(),
      updatedBy: DRIVER_EMAIL,
    });
    showToast(
      isFull
        ? "Marked the coaster as FULL."
        : "Marked the coaster as not full."
    );
  } catch (err) {
    console.error("Failed to update fullness:", err);
    showToast("Could not update coaster capacity status.");
  }
}

function renderDriverStatus() {
  if (!driverCurrentStatus) return;
  driverCurrentStatus.innerHTML =
    `<span>${coasterAvailable ? "✅ Available" : "🚫 Not available"}</span>` +
    `<span>${coasterFull ? "🔴 Full" : "🟢 Not full"}</span>`;
}

// ================= ROLE SELECTION + DRIVER LOGIN UI =================
function showRoleSelect() {
  roleSelectScreen.hidden = false;
}
...
// Lets a driver hand the device back to a student (or vice versa) without
// reloading the page.
...
loginForm.addEventListener("submit", (e) => {
  ...
  if (email === DRIVER_EMAIL && password === DRIVER_PASSWORD) {
    ...
    sessionStorage.setItem(ROLE_SESSION_KEY, "driver");
    isDriverLoggedIn = true;
    closeLoginModal();
    showDriverPanel();
    showToast("Logged in as driver.");
  } else {
...
markAvailableBtn.addEventListener("click", () => setCoasterAvailability(true));
markUnavailableBtn.addEventListener("click", () => setCoasterAvailability(false));
markFullBtn.addEventListener("click", () => setCoasterFull(true));
markNotFullBtn.addEventListener("click", () => setCoasterFull(false));

// Restore role/driver-panel visibility on page load/refresh, based on what
// was chosen earlier in this same browser tab session.
const savedRole = sessionStorage.getItem(ROLE_SESSION_KEY);
if (isDriverLoggedIn && savedRole === "driver") {
  hideRoleSelect();
  showDriverPanel();
...
async function reserveSeat() {
  ...
  if (!coasterAvailable) {
    showToast("The driver has marked this coaster as unavailable right now.");
    return;
  }

  if (coasterFull) {
    showToast("The driver reports the coaster is currently full.");
    return;
  }
  ...
  try {
    const result = await runTransaction(reservationsRef, (current) => {
      ...
      const usedSeats = new Set(
        Object.values(active)
          .map((r) => Number(r?.seatNo))
          .filter((n) => Number.isInteger(n) && n >= 1 && n <= CAPACITY)
      );
      let seatNo = 0;
    ...
    const committedReservations = result.snapshot.val() || {};
    const committedSeatNo = committedReservations?.[trip.key]?.[reservationId]?.seatNo || null;
...
function getSeatMap(trip) {
  const seats = Array.from({ length: CAPACITY }, (_, i) => ({
    seatNo: i + 1,
    reservation: null,
    mine: false,
  }));
  if (!trip) return seats;

  const active = activeReservationsForTrip(trip.key);
  const pending = [];

  Object.entries(active).forEach(([id, reservation]) => {
    const seatNo = Number(reservation?.seatNo);
    if (Number.isInteger(seatNo) && seatNo >= 1 && seatNo <= CAPACITY && !seats[seatNo - 1].reservation) {
      seats[seatNo - 1].reservation = { id, ...reservation };
      seats[seatNo - 1].mine = reservation.deviceId === deviceId;
    } else {
      pending.push([id, reservation]);
    }
  });

  // Older reservations created before numbered seats were introduced still appear correctly.
  for (const [id, reservation] of pending) {
    const free = seats.find((seat) => !seat.reservation);
    if (!free) break;
    free.reservation = { id, ...reservation };
    free.mine = reservation.deviceId === deviceId;
  }
...
function renderSeatGrid(trip) {
  if (!trip) {
    seatGrid.innerHTML = '<div class="seat loading">No trip selected</div>';
    return;
  }

  const seats = getSeatMap(trip);
  seatGrid.innerHTML = "";
  seats.forEach((seat) => {
    const el = document.createElement("div");
    el.className = "seat";
    if (seat.reservation) el.classList.add(seat.mine ? "mine" : "reserved");
    el.setAttribute("aria-label", seat.reservation
      ? `Seat ${seat.seatNo}, ${seat.mine ? "your reservation" : "reserved"}`
      : `Seat ${seat.seatNo}, available`);
    el.innerHTML = `<span>${seat.seatNo}</span><small>${seat.reservation ? (seat.mine ? "YOU" : "TAKEN") : "OPEN"}</small>`;
...
function renderReservation() {
  ...
  reservationTrip.textContent =
    `Seat ${activeReservation.seatNo || "—"} • Departure: ${tripTime} • ${activeReservation.dateKey}`;
...
function render() {
  ...
  if (!firebaseReady && !firebaseError) {
    slotCount.textContent = "Connecting to live seats…";
    tripLabel.textContent = "Loading real-time availability…";
    syncIndicator.textContent = firebaseConnected ? "Live sync" : "Connecting…";
  ...
  if (firebaseError) {
    slotCount.textContent = "Live sync unavailable";
    tripLabel.textContent = "Seat counts cannot be verified right now.";
    syncIndicator.classList.add("offline");
    syncIndicator.textContent = "Offline";
    firebaseErrorBox.hidden = false;
    firebaseErrorMessageEl.textContent = firebaseErrorMessage || "Please check your Firebase Realtime Database settings.";
    reserveBtn.disabled = true;
    tripSelect.disabled = true;
    renderSeatGrid(null);
    return;
  ...
  firebaseErrorBox.hidden = true;
  syncIndicator.classList.remove("offline");
  syncIndicator.textContent = firebaseConnected ? "Live sync" : "Syncing…";
  populateTrips();
  renderSchedule();
  ...
  // "Full" can come from the driver's manual toggle OR from every seat
  // actually being reserved — whichever says full wins.
  const autoFull = stats.available === 0;
  const isFull = coasterFull || autoFull;

  if (!coasterAvailable) {
    availabilityBanner.hidden = false;
    availabilityBanner.classList.remove("full-variant");
    availabilityIcon.textContent = "🚫";
    availabilityMessage.textContent =
      "The driver has marked this coaster as NOT available right now. New reservations are paused.";
  } else if (coasterFull) {
    availabilityBanner.hidden = false;
    availabilityBanner.classList.add("full-variant");
    availabilityIcon.textContent = "🈵";
    availabilityMessage.textContent =
      "The driver reports the coaster is currently full. New reservations are paused until seats free up.";
  } else {
  ...
  if (!coasterAvailable) {
    ...
    tripLabel.textContent = "Coaster marked unavailable by the driver.";
  } else if (trip && isBreak(nowMin)) {
    ...
    statusBox.classList.add("status-full");
    statusIcon.textContent = "🚫";
    tripLabel.textContent = coasterFull
      ? "Driver reports the coaster is currently full."
      : `${formatTripTime(trip.departureMin)} trip is full`;
...
// Reconnect Firebase when a mobile browser comes back online or becomes visible again.
window.addEventListener("online", () => {
  try { goOnline(db); } catch (err) { console.warn("Firebase reconnect failed:", err); }
});
